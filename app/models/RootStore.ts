import { Instance, SnapshotOut, getSnapshot, types } from "mobx-state-tree"
import { Dropzone, DropzoneModel } from "./dropzones"
import { LocationObject } from "expo-location"
import _ from "lodash"
import type { Geojson } from "react-native-maps"
...
export const RootStoreModel = types
  ...
  .actions((self) => ({
    afterCreate() {
      self.dropzones = dropzoneData.features.map((f: Geojson) => {
        return {
          ...f.properties,
  ...
  .views((self) => ({
    dropzoneById(anchor) {
      return getSnapshot(
        _.find(self.dropzones, (d) => parseInt(d.anchor) === parseInt(anchor)),
      ) as Dropzone
    },
    // Gets all the dropzones that have flags
    // get flaggedDropzones () {
    //   if (__DEV__) console.log('flags', self.flags)
    //   if (__DEV__) console.log('', _.filter(self.dropzones, (d) => _.includes(self.flags, d.anchor)))
    ...
    get uniqueAircraft() {
      return _.uniq(
        _.flatMap(self.dropzones, ({ aircraft }) =>
          _.map(aircraft, (plane) => {
            if (plane.match(/[0-9] (.*)/)) {
              const withoutNum = plane.substr(plane.indexOf(" ") + 1)
              return withoutNum.substr(-1) === "s"
                ? withoutNum.substr(0, withoutNum.length - 1)
                : withoutNum
            } else {
              return plane
            }
  ...
  .views((self) => ({
    ...
    get uniqueServices() {
      return _.uniq(_.flatMap(self.dropzones, ({ services }) => _.map(services)))
        .slice()
        .sort()
    },
    // Lists all unique training capabilities
    get uniqueTraining() {
      return _.uniq(_.flatMap(self.dropzones, ({ training }) => _.map(training)))
        .slice()
        .sort()
    },
    ...
    groupByState() {
      return _.groupBy(self.dropzones, "stateOrInternational")
    },
    // Returns all the dropzones grouped by state so you can count how many are in each.
    sortByDistanceFrom(location: LocationObject) {
      if (__DEV__) console.log("sortByDistanceFrom", location)
      return _.sortBy(
        _.map(self.dropzones, (dz) => {
          return {
            ...dz,
